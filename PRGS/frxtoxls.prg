...
&&DO c:\users\yec\documents\fox\vfpp\genselect\portapapeles\genfrxtoxlsreport.prg

PROCEDURE frxtoxls
LPARAMETERS tcGroupField,tcGroupFieldDescrip,tcTable,tcLaTitles,tcAFormat,tcAFields,tcAnchos,tcTitle,tcTipoReporte,tbSumGroup,tcCharSeparator,tbSaveFile,tcFileName
	SET DATE FRENCH 
	&&Crear obejto 
	PUBLIC oExcel 
	...
	oExcel.Visible = .f.
	IF !BETWEEN(VAL(oExcel.version),12,14)  
...
*	oExcel.Sheets("Hoja1").Select()
	...
	*-- Títulos Reporte
	...
	oExcel.Range("A1").Select()
*	oExcel.Selection.Columns("A:A").ColumnWidth=100
	
	IF tbSaveFile
		IF VARTYPE(tcFileName)="C"
			IF !EMPTY(tcFileName)
				IF FILE(tcFileName)=.T.
...
*				MESSAGEBOX(loValue)
				oExcel.ActiveCell.Value =loValue
...
*!*			MESSAGEBOX("lcRange: "+lcRange+" LEN(lcRange)"+TRANSFORM(LEN(lcRange))+" anAnchos(N): "+TRANSFORM(anAnchos(N)))
*!*			oExcel.Selection.Columns(lcRange).ColumnWidth=anAnchos(N)
*!*		ENDFOR	
*!*		
	oExcel.Range("A1").Select()
	FOR lnAnchos=1 TO ALEN(anAnchos)
		*MESSAGEBOX("n: "+TRANSFORM(n)+" anAnchos(N): "+TRANSFORM(anAnchos(N)))
		oExcel.Selection.Columns(lnAnchos).ColumnWidth=anAnchos(lnAnchos)
	ENDFOR
ENDPROC 
